                ...
                >
                  ...
                  <div className=" m-5 grid grid-cols-2 gap-x-64 gap-y-20 py-10">
                    {data.map((itemdata: any) => {
                      if (
                        itemdata.PARSED_MANUAL_TAGS.THEME === themItem ||
                        itemdata.PARSED_MANUAL_TAGS.THEME?.includes(themItem)
                      ) {
                        const level = Array.isArray(
                          itemdata.PARSED_MANUAL_TAGS.CO_DESIGN_LEVEL
                        )
                          ? itemdata.PARSED_MANUAL_TAGS.CO_DESIGN_LEVEL[0]
                          : itemdata.PARSED_MANUAL_TAGS.CO_DESIGN_LEVEL;
                        ...
                        return (
                          <div
                            ...
                            id={itemdata.Key}
                            ref={(ref) =>
                              (elementRefs.current[itemdata.Key] = ref)
                            }
                          ...
                          >
                            <ArcherElement
                              ...
                              relations={
                                itemdata.Extra
                                  ? [
                                      ...itemdata.PARSED_RELATES_TO.map(
                                        (item: any) => {
                                          console.log({ item });
                                          return {
                                            targetId: item,
                                            sourceAnchor: getAnchorPosition(
                                              positions[themItem]?.[
                                                itemdata.Key
                                              ],
                                              positions[themItem]?.[item]
                                            )?.[0],
                                            targetAnchor: getAnchorPosition(
                                              positions[themItem]?.[
                                                itemdata.Key
                                              ],
                                              positions[themItem]?.[item]
