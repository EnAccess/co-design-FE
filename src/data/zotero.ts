...
function parseRelations(extra: string | null | undefined) {
    ...
    return extra.split("RELATES_TO:")[1].split("|").map((item) => item.trim());
}
...
function parseAuthors(creators: any[]) {
    return creators?.filter((creator) => creator.creatorType === "author").map((author) => author.firstName + " " + author.lastName).join("; ");
}
